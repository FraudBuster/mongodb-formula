...
We will see a quick setup to configure the ``mongodb`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``mongodb`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``mongodb`` formulas under ``/srv/salt/mongodb/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/mongodb/parameters/dns:domain/
    ln -s dns:domain /srv/salt/mongodb/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/mongodb/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/mongodb/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``mongodb`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``mongodb`` formulas under ``/srv/salt/mongodb/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/mongodb/parameters/roles
...
We create a configuration for the role ``mongodb/server`` in ``/srv/salt/mongodb/parameters/roles/mongodb/server.yaml``:
...
We create another configuration for the role ``mongodb/client`` in ``/srv/salt/mongodb/parameters/roles/mongodb/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles mongodb/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles mongodb/client
...
.. code-block:: console
    ...
    mv /srv/salt/mongodb/parameters/dns:domain/  '/srv/salt/mongodb/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-mongodb-as-json:
      test.nop:
        - name: {{ mongodb | json }}
...
Here is an example based on `template-formula/mongodb/config/file.sls`_:
...
This ``sls`` file expose a ``mongodb`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ mongodb['winner'] }}
...
.. _template-formula/mongodb/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/mongodb/config/file.sls
